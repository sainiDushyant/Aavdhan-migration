  ...
  return (
    <li
      ...
      <Link className='d-flex align-items-center' to='/' onClick={e => onCollapseClick(e, item)}>
        ...
        {emailAddress && emailAddress === 'intellidemo@grampower.com' && item.title === 'SBPDCL' ? (
          <span className='menu-title text-truncate'>{'IntelliSmart'}</span>
        ) : emailAddress && emailAddress === 'gmr@grampower.com' && item.title === 'SBPDCL' ? (
          <span className='menu-title text-truncate'>{'GMR'}</span>
        ) : (
