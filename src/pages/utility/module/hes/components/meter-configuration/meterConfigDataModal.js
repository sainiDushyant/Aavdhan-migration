...
const getCommunicationProtocolInitialState = (propsRow, protocolOption) => {
  ...
  for (const index in protocolOption) {
    if (passCommunicationProtocol === protocolOption[index]['value']) {
      return {
        value: protocolOption[index]['value'],
        label: protocolOption[index]['value'],
      };
    }
  }
  return undefined;
...
const MeterConfigDataModal = (props) => {
  ...
  const [communication_protocol, setCommunication_Protocol] = useState(() =>
    getCommunicationProtocolInitialState(row, communicationProtocolOptions)
  );
  const [blockload_interval, setBlockLoad_Interval] = useState(() =>
    getBlockLoadIntervalInitialState(row, BlockLoadIntervalOptions)
  );
  const [eventPushIPv6, setEventPushIpv6] = useState(getEventPushIPV6(row));
  const [periodicPushIpv6, setPeriodicPushIpv6] = useState(
    getPeriodicPushIPV6(row)
  );
  const [periodicPushTime, setPeriodicPushTime] = useState(() =>
    getPeriodicPushTime(row)
  );

  const [disableCommunicationProtocol, setDisableCommunicationProtocol] =
    useState(true);
  const [disableBlockLoadInterval, setDisableBlockLoadInterval] =
    useState(true);
  ...
  const [executingCommunicationProtocol, setExecutingCommunicationProtocol] =
    useState(false);
  const [executingBlockLoadInterval, setExecutingBlockLoadInterval] =
    useState(false);
  const [executingEventPushIPV6, setExecutingEventPushIPV6] = useState(false);
  const [executingPeriodicPushIPV6, setExecutingPeriodicPushIPV6] =
    useState(false);
  ...
  const { isFetching, data, isError } = useGetMeterMetaDataQuery(params);
    ...
    if (id === 1) {
      ...
      if (blockload_interval) {
        if (meterMetaInformation) {
          ...
          onUpdateMeterConfigurationButtonClicked(
            createDLMSCommandDataBody(command, args),
            2
          );
        }
      } else {
          ...
          {!executingCommunicationProtocol && (
            <Col lg="6" className="mt-2 ">
              ...
              <Button
                ...
                outline
                color="success"
                onClick={() => onApplyButtonClicked(1)}
              >
                Apply
              </Button>
            </Col>
          ...
          {!executeDLMSCommandResponse.isLoading && (
            <Col lg="6" className="mt-2">
              ...
              <Button
                ...
                outline
                color="success"
                onClick={() => onApplyButtonClicked(2)}
              >
                Apply
              </Button>
            </Col>
          ...
          {!executingEventPushIPV6 && (
            <Col lg="6" className="mt-2">
              ...
              <Button
                ...
                outline
                color="success"
                onClick={() => onApplyButtonClicked(3)}
              >
                Apply
              </Button>
            </Col>
          ...
          {!executingPeriodicPushIPV6 && (
            <Col lg="6" className="mt-2">
              ...
              <Button
                ...
                outline
                color="success"
                onClick={() => onApplyButtonClicked(4)}
              >
                Apply
              </Button>
            </Col>
          ...
          {!executingPeriodicPushTime && (
            <Col lg="6" className="mt-2">
              ...
              <Button
                ...
                outline
                color="success"
                onClick={() => onApplyButtonClicked(5)}
              >
                Apply
              </Button>
            </Col>
