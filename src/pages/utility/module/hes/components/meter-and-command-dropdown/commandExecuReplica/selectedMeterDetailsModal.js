...
const MeterDetailsModal = (props) => {
  ...
  const SelectedPssMeterList = (
    MeterTblDataParam,
    selectedPssRowParam,
    selectedDtrRowParam,
    selectedFeederRowParam
  ) => {
    for (let i = 0; i < MeterTblDataParam.length; i++) {
      if (assetType === 'pss') {
        ...
        if (
          selectedFeederRowParam.feeder_id === MeterTblDataParam[i].feeder_id
        ) {
          return MeterTblDataParam[i].meters;
        }
  ...
  const confirmUpdate = () => {
    if (data.length === 0) {
      const newData = [
        ...MeterTblData.slice(0, rowIndex),
        ...MeterTblData.slice(rowIndex + 1),
      ];
  ...
  return (
    <>
      <SimpleDataTable
        ...
        tableName={'Added Meter Details'}
      />

      {/* Update button */}
      <Button color="primary" className="float-end" onClick={confirmUpdate}>
        Update
      </Button>
    </>
  );
