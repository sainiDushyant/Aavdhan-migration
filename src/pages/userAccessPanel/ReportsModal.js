...
const ReportsModal = (props) => {
  ...
  const createReportsDropdown_New = () => {
    ...
    for (let i = 0; i < props.fetchedReportList.length; i++) {
      if (
        ...
        props.fetchedReportList[i].vertical_name
      ) {
    ...
    for (let i = 0; i < report_dropdown.length; i++) {
      report_dropdown[i][
        'label'
      ] = `${report_dropdown[i]['report_type']}-${report_dropdown[i]['report_name']}`;
  ...
  const [alreadySelectedReport, setAlreadySelectedProject] = useState(
    props.projectLevelAnalyticsReportAccess.filter(
      createAlreadySelectedValues
    )[0].report_access
  );
  ...
  const update = () => {
    ...
    for (let i = 0; i < report_dummy.length; i++) {
      if (
        report_dummy[i].project === props.rowSelectedToEdit.project &&
        report_dummy[i].vertical === props.rowSelectedToEdit.vertical
      ) {
