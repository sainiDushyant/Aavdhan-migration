...
const ModuleModal = (props) => {
  const projectLevelAnalyticsAccess_Local = [
    ...props.projectLevelAnalyticsReportAccess,
  ];
  ...
  const updateRow = () => {
    if (selectedModuleValue.length >= 1) {
      ...
      const analytics_module_previously_existed =
        props.rowSelectedToEdit.module.includes('Analytics');
      ...
      if (
        analytics_module_previously_existed &&
        analytics_module_exists_in_update
      ) {
        ...
        const fetchedReportList_Dummy = props.fetchedReportList.filter(
          (e) => e.vertical_name === props.rowSelectedToEdit.vertical
        );
        const temp = {
        ...
        for (let i = 0; i < projectLevelAnalyticsAccess_Local.length; i++) {
          if (
            projectLevelAnalyticsAccess_Local[i].vertical ===
              props.rowSelectedToEdit.vertical &&
            projectLevelAnalyticsAccess_Local[i].project ===
      ...
      toast(' At least 1 module must exist  .', {
